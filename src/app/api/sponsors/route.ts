...
/**
 * Server-side proxy for the sponsors endpoint. The backend sends no CORS
...
export async function GET() {
  try {
    const res = await fetch(
      "https://mining-investment-backend.vercel.app/api/sponsors?limit=200",
      {
        headers: {
    ...
    console.error("Proxy Sponsors Error:", error);
    return NextResponse.json(
      {
        success: false,
